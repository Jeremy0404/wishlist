...
router.post('/register', async (req, res) => {
    ...
    const [user] = await db('users').insert({ email, password_hash, name }).returning(['id', 'email']);

    // Auto-create a family for the first user? Optional. We’ll let them create/join manually.

    const token = signUser({ id: user.id, email: user.email });
    res.cookie(authCookie.name, token, authCookie.options).json({ id: user.id, email: user.email });
});
