...
import { authRequired, familyContext, mustHaveWishlistWithItem } from '../middleware/auth.js';

const router = Router();

// Ensure you have a wishlist in current family; create if missing
router.post('/me', authRequired, familyContext, async (req, res) => {
    const existing = await db('wishlists').where({ user_id: req.user!.id, family_id: req.familyId! }).first();
    if (existing) return res.json(existing);
    const [wl] = await db('wishlists').insert({ user_id: req.user!.id, family_id: req.familyId! }).returning('*');
    res.json(wl);
});
...
router.post('/me/items', authRequired, familyContext, async (req, res) => {
    ...
    let wl = await db('wishlists').where({ user_id: req.user!.id, family_id: req.familyId! }).first();
    if (!wl) [wl] = await db('wishlists').insert({ user_id: req.user!.id, family_id: req.familyId! }).returning('*');
...
router.patch('/me/items/:id', authRequired, familyContext, async (req, res) => {
    ...
    const row = await db('wishlist_items as i').join('wishlists as w', 'w.id', 'i.wishlist_id')
        .where({ 'i.id': id, 'w.user_id': req.user!.id, 'w.family_id': req.familyId! }).first();
    if (!row) return res.status(404).json({ error: 'not found' });

    const parse = Item.partial().safeParse(req.body);
    if (!parse.success) return res.status(400).json(parse.error.flatten());

    const [updated] = await db('wishlist_items').where({ id }).update(parse.data).returning('*');
    res.json(updated);
});

router.delete('/me/items/:id', authRequired, familyContext, async (req, res) => {
    const { id } = req.params;
    const owned = await db('wishlist_items as i').join('wishlists as w', 'w.id', 'i.wishlist_id')
        .where({ 'i.id': id, 'w.user_id': req.user!.id, 'w.family_id': req.familyId! }).first();
...
router.get('/', authRequired, familyContext, mustHaveWishlistWithItem, async (req, res) => {
    const rows = await db('wishlists as w')
        .join('users as u', 'u.id', 'w.user_id')
        .where('w.family_id', req.familyId!)
        .andWhereNot('w.user_id', req.user!.id)
        .select('w.id as wishlist_id', 'u.id as user_id', 'u.name', 'w.created_at')
        .orderBy('u.name', 'asc');
    res.json(rows);
});
...
router.post('/items/:id/reserve', authRequired, familyContext, async (req, res) => {
    const { id } = req.params;
    ...
    const myWl = await db('wishlists').where({ user_id: req.user!.id, family_id: req.familyId! }).first();
    if (myWl && myWl.id === row.wishlist_id) return res.status(400).json({ error: 'cannot reserve your own item' });

    const existing = await db('reservations').where({ item_id: id }).first();
    if (existing) return res.status(409).json({ error: 'already reserved' });

    const [r] = await db('reservations').insert({
        item_id: id,
...
router.post('/items/:id/unreserve', authRequired, familyContext, async (req, res) => {
    const { id } = req.params;
    const r = await db('reservations').where({ item_id: id, reserver_user_id: req.user!.id }).first();
    if (!r) return res.status(404).json({ error: 'not reserved by you' });
    await db('reservations').where({ id: r.id }).del();
    res.json({ ok: true });
});

router.post('/items/:id/purchase', authRequired, familyContext, async (req, res) => {
    const { id } = req.params;
    const r = await db('reservations').where({ item_id: id, reserver_user_id: req.user!.id }).first();
    if (!r) return res.status(404).json({ error: 'not reserved by you' });
    const [u] = await db('reservations').where({ id: r.id }).update({ status: 'purchased' }).returning('*');
    res.json(u);
});
