...
export async function seed(knex: Knex): Promise<void> {
    ...
    const [noelFamily, summerFamily] = await knex('families')
        .insert([
            { id: randomUUID(), name: 'Fam Noel', invite_code: 'NOEL-2025' },
    ...
    const [alice, bob, charlie, diana] = await knex('users')
        .insert([
            { id: randomUUID(), email: 'alice@example.com', password_hash, name: 'Alice' },
            { id: randomUUID(), email: 'bob@example.com', password_hash, name: 'Bob' },
            { id: randomUUID(), email: 'charlie@example.com', password_hash, name: 'Charlie' },
            { id: randomUUID(), email: 'diana@example.com', password_hash, name: 'Diana' }
        ])
        .returning('*');
